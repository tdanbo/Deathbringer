...
class CombatLogGUI:
    ...
    def get_widget_directory(self):
        ...
        for entry in range(10):
            entry_ui = self.create_log_entry2(self.log_scroll.inner_layout(0))
            # self.log_dictionary[entry] = {"character":entry_ui[0],"icon":entry_ui[1],"type":entry_ui[2],"roll":entry_ui[3],"time":entry_ui[4]}
        latest_entry_ui = self.create_log_entry2(self.log_latest.inner_layout(0))
    ...
    def create_log_entry2(self, layout):
        print(layout)
        print("2")
        self.log_layout = Section (
            outer_layout = QVBoxLayout(),
